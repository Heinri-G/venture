...
> **Venture project override (dominant source of truth).** This project already
> has a committed brand identity and design system that outrank every generic rule
> in this skill: `AGENTS.md`, `.agents/rules/design_guidelines.md`, and the
> incumbent visual world (edelweiss mark, Nunito Sans, sage/meadow oklch tokens,
> shadcn/ui + Radix primitives, pill buttons/sheets, `--radius: 1rem`,
> `--ease-venture`). Use this skill to fight slop and inform direction, but honor
> the project's tokens, shadcn/ui conventions, and established identity first.
> `lucide-react` is the project's established icon family; do not swap it. The app
> is Vite + React + Tailwind v4, not Next.js — ignore RSC/`next/font` guidance.
