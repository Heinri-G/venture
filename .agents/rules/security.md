...
4. **Client vs Server Secrets**:
   ...
   - Backend keys must NOT have a `VITE_` prefix and must only be used in Netlify Functions or other server-side code (e.g. `SUPABASE_SERVICE_ROLE_KEY`).
5. **Proxy Secrets Through Netlify Functions**: Proxy third-party APIs through Netlify Functions under `netlify/functions/` so secret keys never reach the client. Never call those APIs directly from the browser with a secret key. (Google Maps share links are parsed without an API key; only short-link redirects go through a function.)
